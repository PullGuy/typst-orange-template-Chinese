#let my-outline-row(
  ...
  if full-width {
    // If fullWidth is true, create a single box for the title,
    // making it start from the far left.
    ...
    // Original behavior for all other entries
...
#let my-outline(
  ...
  show outline.entry: it => {
    ...
    if it.level == 1 {
      ...
      if (part-change) {
        v(0.7cm, weak: true)
        box(width: 1.1cm, fill: main-color.lighten(80%), inset: 5pt, align(center, text(
          size: textSize1,
          weight: "bold",
        ...
        h(0.1cm)
        box(width: 100% - 1.2cm, fill: main-color.lighten(60%), inset: 5pt, align(center, link(part-location, text(
          size: textSize1,
          weight: "bold",
          part-state,
        ))))
