  ...
  [
    ...
    #context {
      ...
      if part-style == 0 [
        ...
        #place(block(width: 100%, height: 100%, outset: (x: 3cm, bottom: 2.5cm, top: 3cm), fill: main-color.lighten(
          70%,
        )))
      ...
      align(bottom + right, my-outline-small(
        title,
        appendix-state,
        part-state,
        part-location,
...
#let appendices(title, doc, hide-parent: false) = {
  ...
  appendix-state.update(x => title)
  appendix-state-hide-parent.update(x => hide-parent)
...
#let book(
  ...
  set page(paper: paper-size, margin: (x: 3cm, bottom: 2.5cm, top: 3cm), header: context {
    ...
    if odd_page {
      ...
      if before != () and counterInt.len() > 1 {
        box(width: 100%, inset: (bottom: 5pt), stroke: (bottom: 0.5pt))[
          #text(if appendix != none { numbering("A.1", ..counterInt.slice(0, 2)) + " " + before.last().body } else {
      ...
      if before != () and counterInt > 0 {
        box(width: 100%, inset: (bottom: 5pt), stroke: (bottom: 0.5pt))[
          #page_number
          #h(1fr)
          #text(weight: "bold", if appendix != none { numbering("A.1", counterInt) + ". " + before.last().body } else {
    ...
    if it.level == 1 {
      ...
      context {
        ...
        if img != none {
          ...
          place(move(dx: -3cm, dy: -3cm, block(width: 21cm, height: 9.4cm, align(right + bottom, pad(
            bottom: 1.2cm,
            block(
              width: 86%,
              stroke: (
